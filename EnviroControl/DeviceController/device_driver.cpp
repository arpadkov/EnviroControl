...
void DeviceDriver::initialize() const
{
  ...
  try
  {
    ...
    _open_line->request({ QString("qt-gpio-control-open-%1").arg(_id).toStdString(),
                     gpiod::line::DIR_OUTPUT, 0 }, // Default flags
      _active_high ? 0 : 1); // Value for OFF state (0 for active-HIGH, 1 for active-LOW)
    ...
    _close_line->request({ QString("qt-gpio-control-close-%1").arg(_id).toStdString(),
                     gpiod::line::DIR_OUTPUT, 0 }, // Default flags
      _active_high ? 0 : 1); // Value for OFF state (0 for active-HIGH, 1 for active-LOW)
